...
Guide dédié aux assistants IA pour le développement du projet E2I VoIP avec Next.js 16, DaisyUI et Vercel. Ce projet est une refonte du site web existant https://www.e2i-voip.com

> 🇫🇷 **Langue** : toujours répondre en français à l'utilisateur.
>
> 👤 **Profil utilisateur** : lire `ME.md` en début de session (façon de travailler, attentes, périmètre de décision).
>
> 📌 **Mémoire projet** : consulter `memory.md` à la racine pour le suivi global (décisions, contexte, intégrations, historique des sessions). À mettre à jour à chaque décision structurante.
>
> 🏛 **Décisions d'architecture** : `docs/ADR.md` — une entrée par décision structurante (contexte, décision, conséquences, tests).
...
3. Les 3 fichiers de charte (`CHARTE_GRAPHIQUE.md`, `Charte_Graphique_E2I_VoIP.docx`, `BrandBrief_e2ivoip.md`) sont en **permission requise** : ne jamais les modifier sans accord explicite.
4. Toute modification de la charte doit être propagée dans les 3 fichiers ET dans `memory.md`.

## 🚀 Stack & Priorities

- **Framework**: NextJS 15 (App Router)
- **CSS**: Tailwind + shadcn/ui (pas DaisyUI en production)
- **Animations**: CSS/Tailwind en priorité ; Framer Motion lazy loaded si motion avancée
- **Icons**: Phosphor Icons via `@/lib/icons`
- **State Management**: useState natif ; Zustand si état global multi-composants
- **Server State**: fetch natif pour les mutations simples
- **Validation**: validation native HTML5 + logique métier légère
- **Deployment**: Vercel
...
- J'ai un **TDA** (trouble du déficit de l'attention), pas seulement TDAH. Explications simples et techniques avancées doivent coexister sans surcharge cognitive.
- Décompose les taches complexes en petite succession de taches simples
- Réponses courtes, numérotées, 3 étapes maximum. Résumé de ce qui vient d'être fait avant la suite.
- **Brainstorming / exploration** : poser les questions une par une, dans un ordre logique, en partant de la réponse précédente. Jamais plusieurs questions ouvertes en vrac dans un même message.
...
**Merge** : l'agent ne merge jamais de sa propre initiative — il livre l'URL de la PR et s'arrête. Sur demande explicite d'Alban, il merge, à condition que les checks CI soient verts : `--squash` vers `dev`, `--merge` (classique) pour `dev → main` afin de ne pas faire diverger les deux branches. Pour `dev → main`, vérifier au préalable l'écart de commits (`git log origin/main..origin/dev`) et l'annoncer.

**Interdictions strictes pour l'agent** : ne jamais pousser directement sur `dev` ou `main` ; ne jamais `--force` sur une branche partagée ; ne jamais utiliser `--admin` ou `--auto` pour contourner un check qui échoue.
...
│   │   ├── forms/                # Form hooks (à créer si besoin)
│   │   └── ui/                   # UI hooks
...
│   ├── ARCHITECTURE.md           # ✅ Architecture + perf + workflow validation
│   ├── PRD.md                    # Product Requirements
│   └── CHARTE_GRAPHIQUE.md       # Couleurs officielles (source de vérité)
