...
- **Point de vigilance** : la propriété HubSpot `last_studio_request` n’est pas standard. Si elle n’existe pas dans le portail, l’upsert échoue en 400 — visible en logs (`[studio/devis] HubSpot sync failed`), sans impact sur l’email. À créer côté HubSpot avant mise en production.
