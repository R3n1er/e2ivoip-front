...
jest.mock('@/components/devis-animations', () => ({
  ...
  AnimatedCard: ({ children, className, delay }: any) => (
    <div className={className} data-testid="animated-card" data-delay={delay}>{children}</div>
  ),
  AnimatedGrid: ({ children, className }: any) => (
    <div className={className} data-testid="animated-grid">{children}</div>
  ),
}))
...
describe('Page Devis En Ligne - Test d\'hydratation', () => {
  ...
  it('affiche les boutons de demande de devis', () => {
    ...
    const buttons = screen.getAllByText('Demander un devis')
    expect(buttons).toHaveLength(4) // Un pour chaque type de devis
  })
