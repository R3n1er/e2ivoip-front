...
describe("Header avec sous-menus DaisyUI", () => {
  ...
  test("Les liens de navigation principaux sont présents", () => {
    ...
    expect(screen.getByTestId("nav-link-mobilité")).toBeInTheDocument();
  ...
  test("Les sous-menus téléphonie s'affichent correctement", async () => {
    ...
    const telephonieDrop = screen.getByTestId("nav-dropdown-téléphonie-d'entreprise");
    await user.hover(telephonieDrop);
    
    await waitFor(() => {
      expect(screen.getByTestId("submenu-téléphonie-d'entreprise")).toBeInTheDocument();
    });
  ...
  test("Le menu mobile DaisyUI fonctionne", async () => {
    ...
    expect(screen.getByTestId("mobile-link-qui-sommes-nous")).toBeInTheDocument();
    expect(screen.getByTestId("mobile-link-mobilité")).toBeInTheDocument();
  ...
  test("Les sous-menus mobiles s'affichent", () => {
    ...
    expect(screen.getByTestId("mobile-submenu-nos-certifications")).toBeInTheDocument();
    expect(screen.getByTestId("mobile-submenu-trunk-sip-au-compteur")).toBeInTheDocument();
    expect(screen.getByTestId("mobile-submenu-studio-attente-téléphonique")).toBeInTheDocument();
  });
  ...
  test("L'accessibilité des icônes est respectée", () => {
    ...
    const mobileMenuIcon = screen.getByLabelText(/Ouvrir le menu|Fermer le menu/);
  ...
  test("Les styles DaisyUI sont appliqués", () => {
    ...
    expect(screen.getByTestId("header-contact-button")).toHaveClass("btn", "btn-primary");
    expect(screen.getByTestId("mobile-contact-button")).toHaveClass("btn", "btn-primary");
    expect(screen.getByTestId("mobile-menu-button")).toHaveClass("btn", "btn-square", "btn-ghost");
  });
  ...
  test("Les liens pointent vers les bonnes URLs", () => {
    ...
    expect(screen.getByTestId("header-contact-button").closest("a")).toHaveAttribute("href", "/contact");
    expect(screen.getByTestId("mobile-contact-button").closest("a")).toHaveAttribute("href", "/contact");
  });

  test("Les sous-menus se ferment après un délai", async () => {
    jest.useFakeTimers();
    render(<Header />);
    ...
    // Quitter le hover
    await user.unhover(quiSommesNousLink);
    
    // Avancer le timer pour déclencher la fermeture
    jest.advanceTimersByTime(300);
    
    await waitFor(() => {
      expect(screen.queryByTestId("submenu-qui-sommes-nous")).not.toBeInTheDocument();
    });
  ...
  test("La transformation de l'icône chevron fonctionne", async () => {
    ...
    const quiSommesNousLink = screen.getByTestId("nav-link-qui-sommes-nous");
    const chevronIcon = screen.getAllByTestId("icon-chevron-down")[0];
