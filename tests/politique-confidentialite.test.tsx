...
describe("Page Politique de Confidentialité", () => {
  ...
  it("contient les sections principales", () => {
    ...
    expect(
      screen.getByText(/Identité du responsable du traitement/)
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Collecte et utilisation des données/)
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Hébergement et sécurité des données/)
    ).toBeInTheDocument();
  ...
  it("contient la certification 3CX", () => {
    ...
    expect(
      screen.getByRole("link", { name: /Découvrir 3CX/i })
    ).toHaveAttribute(
      "href",
      "https://www.3cx.fr/pabx/download-pabx-ip/?resellerId=208715"
    );
  });
